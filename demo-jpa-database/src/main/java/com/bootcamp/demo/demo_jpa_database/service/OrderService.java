...
import com.bootcamp.demo.demo_jpa_database.repository.ItemRepository;
...
public class OrderService {
  ...
  public OrderEntity checkout(List<OrderItemEntity> items) {
    // payment step
    // clear cart
    Double orderAmount = items.stream().mapToDouble(item -> item.getPrice() * item.getQuantity()).sum();
    ...
    this.itemRepository.saveAll(items);// many insert
